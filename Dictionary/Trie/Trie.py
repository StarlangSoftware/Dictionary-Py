...
class Trie:
    ...
    def addWord(self, word : str, root : Word):
        self.rootNode.addWord(word, root)

    """
    The getWordsWithPrefix method which takes a String surfaceForm as an input. First it creates a TrieNode current and 
    assigns the rootNode to it, then it creates a new set words. It loops i times where i ranges from 0 to length of 
    surfaceForm and assigns current's child that corresponds to the surfaceForm's char at index i and assigns it as 
    TrieNode current. If current is not None, it adds all words of current to the words set.
    ...
    child that corresponds to the hash's char at index i and assigns it as current. If current is None, it returns null.
